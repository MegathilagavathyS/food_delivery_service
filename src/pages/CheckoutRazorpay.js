...
import { buildUrl, getAuthToken, USE_MOCK_API } from '../config/api';
...
const CheckoutRazorpay = () => {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      // If Razorpay returned an OTP-specific validation error, offer retry
      const razorpayError = err.response?.data?.error || err.response?.data;
      if (razorpayError && (razorpayError.field === 'otp' || /otp/i.test(razorpayError.description || ''))) {
        // Prompt user to retry entering OTP via the widget
        if (paymentRetryCount < PAYMENT_MAX_RETRIES) {
          const tryAgain = window.confirm('Payment failed due to incorrect OTP. Would you like to try again?');
          if (tryAgain) {
            setPaymentRetryCount(paymentRetryCount + 1);
            // Retry flow: re-run payment initiation which will recreate order and reopen widget
            setTimeout(() => handleProceedToPayment(), 200);
            return;
          }
        } else {
          alert('Payment failed repeatedly due to incorrect OTP. Please try a different payment method or contact support.');
        }
      } else {
        alert(errorMsg);
      }
